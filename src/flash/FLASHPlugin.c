...
static int loaded_plugin_load(struct target *target, struct advanced_elf_image *image, struct loaded_plugin *plugin, uint32_t init_done_address, unsigned stackSize)
{
	...
	LOG_INFO("FLASH plugin: placing the stack at 0x%08x-0x%08x", lastSectionEnd, lastSectionEnd + stackSize);
...
FLASH_BANK_COMMAND_HANDLER(plugin_flash_bank_command)
{
    ...
    LOG_INFO("FLASH plugin: FLASHPlugin_InitDone = 0x%08x", info->FLASHPlugin_InitDone);
    LOG_INFO("FLASH plugin: FLASHPlugin_Probe = 0x%08x", info->FLASHPlugin_Probe);
    LOG_INFO("FLASH plugin: FLASHPlugin_FindWorkArea = 0x%08x", info->FLASHPlugin_FindWorkArea);
    LOG_INFO("FLASH plugin: FLASHPlugin_EraseSectors = 0x%08x", info->FLASHPlugin_EraseSectors);
    LOG_INFO("FLASH plugin: FLASHPlugin_Unload = 0x%08x", info->FLASHPlugin_Unload);
    LOG_INFO("FLASH plugin: FLASHPlugin_ProgramSync = 0x%08x", info->FLASHPlugin_ProgramSync);
    LOG_INFO("FLASH plugin: FLASHPlugin_ProgramAsync = 0x%08x", info->FLASHPlugin_ProgramAsync);
    LOG_INFO("FLASH plugin: FLASHPlugin_NotImplemented = 0x%08x", info->FLASHPlugin_NotImplemented);
    LOG_INFO("FLASH plugin: FLASHPlugin_CheckSectorProtection = 0x%08x", info->FLASHPlugin_CheckSectorProtection);
    LOG_INFO("FLASH plugin: FLASHPlugin_ProtectSectors = 0x%08x", info->FLASHPlugin_ProtectSectors);
    LOG_INFO("FLASH plugin: done");
    ...
    if (retval == ERROR_OK)
    {
        ...
        else if (plugin_info->FLASHPlugin_ProgramSync && plugin_info->FLASHPlugin_ProgramSync != plugin_info->FLASHPlugin_NotImplemented)
        {
            ...
            while (done < count)
            {
                ...
                if (doneNow < 0)
                {
                    LOG_ERROR("plugin_write_sync returned error (%u)", doneNow);
                    retval = ERROR_FLASH_BANK_INVALID;   
                    break;
                }
